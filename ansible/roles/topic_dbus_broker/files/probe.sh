...
#
# probe.sh — read-only diagnostics for topic_dbus_broker.
#
# Reports current state of the dbus-broker.service unit, the two drop-in
...
readonly DROPIN_DIR="/etc/systemd/system/dbus-broker.service.d"
readonly CIL_DIR="/usr/local/share/selinux"
readonly CIL_MODULE="nnp_dbus_broker"
readonly LAUNCHER_PATH="/usr/bin/dbus-broker-launch"
readonly BROKER_PATH="/usr/bin/dbus-broker"
readonly STOCK_UNIT_PATH="/usr/lib/systemd/system/dbus-broker.service"
...
probe_stock_unit_directives() {
  ...
  grep -E '^(RuntimeDirectory|StateDirectory|ConfigurationDirectory|LogsDirectory|ProtectSystem|ProtectHome|ProtectKernel|ProtectControl|ProtectClock|ProtectHostname|ProtectProc|PrivateTmp|PrivateDevices|PrivateUsers|RestrictNamespaces|RestrictRealtime|RestrictSUIDSGID|RestrictAddressFamilies|LockPersonality|MemoryDenyWriteExecute|SystemCallFilter|SystemCallArchitectures|DeviceAllow|CapabilityBoundingSet|NoNewPrivileges|AmbientCapabilities|UMask|ProcSubset|User|Group|Type|ExecStart|BusName|Sockets)=' \
...
probe_merged_unit() {
  ...
  systemctl cat "${UNIT}" 2>&1 \
    | grep -E '99-(hardening|nnp)\.conf|NoNewPrivileges|ProtectClock|ProtectKernelLogs|ProtectKernelModules|ProtectControlGroups|SystemCallArchitectures|MemoryDenyWriteExecute' \
    || true
}
...
probe_fcontext() {
  ...
  matchpathcon "${LAUNCHER_PATH}" 2>/dev/null || true
  matchpathcon "${BROKER_PATH}" 2>/dev/null || true
}

probe_busctl_list() {
  printf -- '--- busctl --system list smoketest ---\n'
  if ! command -v busctl >/dev/null 2>&1; then
    printf 'busctl: not available\n'
    return
  fi
  busctl --system list 2>&1 || true
}

probe_dbus_send_roundtrip() {
  printf -- '--- dbus-send round-trip smoketest ---\n'
  if ! command -v dbus-send >/dev/null 2>&1; then
    printf 'dbus-send: not available\n'
    return
  fi
  dbus-send --system --print-reply \
    --dest=org.freedesktop.DBus \
    /org/freedesktop/DBus \
    org.freedesktop.DBus.ListNames 2>&1 || true
}
