...
#
# verify.sh — Soll/Ist comparison for topic_flatpak_audio_sandbox.
#
# Compares observed runtime state against the expected end-state
# declared in docs/reference/topics/flatpak-audio-sandbox.md. The
# end-state is the priority-400 module flatpak_audio_sandbox loaded
# with the single allow surface present in the loaded policy:
#   - functional: staff_t × device_t : dir mounton
...
# live bwrap process. Where any liveness inspection is required
...
#
# The AVC-clean check reports zero on a host that has never launched
# an audio-permission-holding Flatpak application since boot as well
# as on a soaked, applied host. The verify does not distinguish these
# two cases — the probe's flatpak inventory output is the
...
verify_rule() {
  ...
  if [[ "${actual}" == "${EXPECTED_RULE_PRESENT}" ]]; then
    report_ok "rule_present" \
      "expected=${EXPECTED_RULE_PRESENT} actual=${actual} (staff_t × device_t : dir mounton)"
  else
...
verify_avc_class_clean() {
  ...
  hits=$(ausearch -m AVC,USER_AVC -ts boot 2>/dev/null \
           | grep -cE '(staff_t.*device_t.*dir|/newroot/dev/snd)' \
           || true)
