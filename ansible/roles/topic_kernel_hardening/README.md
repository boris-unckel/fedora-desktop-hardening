...
- The `restorecon kernel-hardening` handler fires on any drop-in change and is a no-op on a correctly installed file. It covers the `coredump.conf.d/` directory as well as the files, because this role creates that directory and a directory under `/etc/systemd` inherits `etc_t` while `file_contexts` maps the path to `systemd_conf_t`. `-F` also resets the SELinux user field. The three stock directories (`sysctl.d`, `modprobe.d`, `limits.d`) are deliberately not relabelled recursively — they hold files belonging to other packages.
