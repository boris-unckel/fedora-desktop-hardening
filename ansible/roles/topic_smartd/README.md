...
| `topic_smartd_fcontext_path` | `/usr/bin/smartd` | F44 fcontext mapping target path. |
| `topic_smartd_fcontext_setype` | `fsdaemon_exec_t` | F44 fcontext mapping SELinux type. |
| `topic_smartd_fcontext_expected` | `system_u:object_r:fsdaemon_exec_t:s0` | Expected `matchpathcon` output for the binary. |
...
- `foundation_sudo_roles` (Layer 1) — the preflight `sesearch` query, the CIL install, the fcontext mapping, the `restorecon` handler, the AVC-clean read, and the live SELinux-domain read all transit through `sudo -r sysadm_r -t sysadm_t`.
...
- `apply` — drop-in deployment, CIL install, fcontext mapping, and live-state read.
...
- `community.general.sefcontext` with `state: present` is idempotent: it adds the fcontext entry on first run and reports unchanged on subsequent runs.
...
- The `restorecon smartd` handler fires only on an fcontext entry change.
