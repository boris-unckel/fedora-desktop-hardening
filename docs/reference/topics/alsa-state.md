...
Stock targeted policy on Fedora 44 or later carries a service-specific subtype `alsa_unit_file_t` and a type-transition `init_t → alsa_unit_file_t : file create` for files under `/etc/systemd/system/alsa-state.service.d/`. The role's `restorecon` after `ansible.builtin.copy` is what triggers the relabel from the install-time default to the unit-specific type.
